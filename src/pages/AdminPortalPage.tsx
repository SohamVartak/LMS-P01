...
export const AdminPortalPage: React.FC<Props> = ({ onOpenBooks, onOpenCirculation, onOpenHealth }) => {
  ...
  const [librarySeats, setLibrarySeats] = useState<any[]>([]);
  ...
  const load = async () => {
    ...
    if (!librarySeatRes.error) {
      setLibrarySeats(librarySeatRes.data || []);
    }
